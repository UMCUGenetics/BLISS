...
blacklist = "/Users/avanhoeck//hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/blissNPanalysis/hg19-blacklist.v2.bed.gz"
...
sampleTable = data.table(name = c("TK6_DMSO_1", "TK6_DMSO_2", "TK6_ETO_1", "TK6_ETO_2"),
                         ...
                         Replicate = c("1", "2", "1", "2"),
                         path=c("/Users/avanhoeck/hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/blissNPanalysis/data/TK6_DMSO_rep1.bed.gz",
                                "/Users/avanhoeck/hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/blissNPanalysis/data/TK6_DMSO_rep2.bed.gz",
                                "/Users/avanhoeck/hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/blissNPanalysis/data/TK6_ETO_rep1.bed.gz",
                                "/Users/avanhoeck/hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/blissNPanalysis/data/TK6_ETO_rep2.bed.gz"))
...
genes <- getBM(attributes=c('chromosome_name','start_position','end_position','strand', 'gene_biotype',
                            'ensembl_gene_id'),filters = 'chromosome_name', values = seqnames(chrom_sizes),   mart = ensembl)
...
pl$TK6_ETO_2$end <- as.numeric(pl$TK6_ETO_2$end)
pl$TK6_ETO_2$start <- as.numeric(pl$TK6_ETO_2$start)

png(filename=file.path("/Users/avanhoeck/Desktop/fig1d_human.png"), units="in", width=8, height=8, res=300)
    ...
    for( batch in sampleTable[, unique(Batch)] ){
        ...
        for( id in sampleTable[Batch==batch, name] ){
            for( chrom in pl[[id]][, unique(chr)] )
                circos.genomicLines(region=pl[[id]][chr==chrom,c("start","end","value")], value=pl[[id]][chr==chrom,c("start","end","value")],
                                    numeric.column=3, col = sel_col[2*(i-1)+ii], track.index=i+2,
                                    sector.index=chrom)
            ii = ii+1
