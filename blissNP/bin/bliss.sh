...
cd /hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/BLISS
...
tmp=$datadir/$experiment/$sample/tmp && mkdir -p $tmp
export HOME=$datadir/$experiment/$sample/

if [ $genome == human ]; then
    #CHANGE THIS TO THE CORRECT LOCATION/INDEX_BASENAME
    refgen=/hpc/cuppen/shared_resources/genomes/GRCh37/Sequence/genome.fa # modify if necessary
fi
...
python /hpc/cuppen/projects/P0008_brca1/dsb_seq/analysis/Arne/BLISS/scripts/filter_bliss_fasta.py -i $in/r1.fa --sample_barcode $barcode --UMI_length $UMIlength > $out/filtered.r1.fa
