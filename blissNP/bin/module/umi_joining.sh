...
if [[ $numb_of_files == 1 ]]; then # IF SE READS
    ...
    LC_ALL=C join $aux/id.chr.loc.strand $aux/id.umi.barcode.genomic | cut -d' ' -f-5 |LC_ALL=C sort --parallel=8 --temporary-directory=$HOME/tmp -t' ' -k2,2 -k3,3n -k4,4 -k5,5 | awk '{print $2,$3,$3+1,$4,$5,$1}' | tr " " "\t" > $out/_q$quality.bed 
fi
